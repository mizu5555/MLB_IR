...
class LookupEngine:
    def __init__(self):
        """
        載入 training_data.json，並維持 record_key → record 的 dict。
        """
        data_path = os.path.join("data", "mlb_data_adv", "training_data.json")

        with open(data_path, "r", encoding="utf-8") as f:
            self.records = json.load(f)

        # 建立 record_key → record 查詢索引（加速後續 lookup）
        self.by_key = {rec["record_key"]: rec for rec in self.records}
