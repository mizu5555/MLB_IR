"""
Phase 4 Enhanced: 整合完整 Statcast 數據
支援所有進階指標的整合
"""
...
def load_data():
    ...
    try:
        ...
        # 統計欄位數
        sample_batter = next((v for v in statcast_data.values() if v.get('type') == 'batter'), None)
        sample_pitcher = next((v for v in statcast_data.values() if v.get('type') == 'pitcher'), None)
        
        if sample_batter:
            print(f"   打者欄位: {len(sample_batter)} 個")
        if sample_pitcher:
            print(f"   投手欄位: {len(sample_pitcher)} 個")
            
    except FileNotFoundError:
        print("❌ 增強 Statcast 數據未找到")
        print("   請先執行: python phase4_download_statcast_enhanced.py")
        return None, None
...
def integrate_statcast(documents, statcast_data: Dict):
    ...
    for doc_id, doc in working_docs.items():
        ...
        if not matched_statcast:
            for stat_name, years_data in statcast_by_player.items():
                if match_player_name(player_name, stat_name):
        ...
        # 整合數據
        if matched_statcast:
            # 建立 statcast 字段，排除 metadata
            statcast_fields = {}
            
            for key, value in matched_statcast.items():
                if key not in ['player_name', 'year', 'type']:
                    statcast_fields[key] = value
            
            # 添加到文檔
            if 'statcast' not in doc:
                doc['statcast'] = {}
            
            doc['statcast'].update(statcast_fields)
            match_count += 1
...
def save_integrated_data(documents):
    ...
    output_file = './mlb_data/week5_mlb_documents_final_enhanced.json'
    ...
    if isinstance(documents, list):
        ...
        for doc in documents:
            if 'statcast' in doc and doc['statcast']:
                total_fields += len(doc['statcast'])
                count += 1
        avg_fields = total_fields / count if count > 0 else 0
    else:
    ...
    print(f"  平均欄位數: {avg_fields:.1f} 個/球員")
...
def show_examples(documents):
    ...
    for i, (doc_id, doc) in enumerate(players_with_statcast[:3], 1):
        ...
        if statcast:
            ...
            # 顯示部分重要欄位
            important_fields = [
                'EV', 'LA', 'Barrel%', 'HardHit%', 'xwOBA', 'xBA', 'xSLG',
                'K%', 'BB%', 'Whiff%', 'Chase%', 'Sprint Speed',
                'wOBA', 'wRC+', 'WAR', 'OPS'
            ]
            
            print("重要指標:")
            for field in important_fields:
                if field in statcast:
                    val = statcast[field]
...
def main():
    ...
    print("=" * 80)
    print("Phase 4 Enhanced: 整合完整 Statcast 數據")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("✨ Phase 4 Enhanced 完成！")
    print("=" * 80)
    print("\n系統現在包含所有可用的進階棒球數據:")
    print("  ✅ 完整打者 Statcast 數據（擊球質量、預期數據、紀律性等）")
    print("  ✅ 完整投手 Statcast 數據（球種、球速、控球、預期數據等）")
    print("  ✅ 所有可用的 FanGraphs 進階指標")
    print("\n下一步:")
    print("  python phase5_rebuild_vector_db.py  # 重建 Vector Database")
